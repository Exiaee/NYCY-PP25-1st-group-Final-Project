...
__device__ float get_C(int i, int N) {
    return (i == 0) ? sqrtf(1.0f / N) : sqrtf(2.0f / N);
}

__global__ void dct2d_kernel(const float* input, float* output, int N) {
    ...
    for (int x = 0; x < N; ++x) {
        for (int y = 0; y < N; ++y) {
            ...
            sum += val *
                   cosf((PI * (2 * x + 1) * u) / (2.0f * N)) *
                   cosf((PI * (2 * y + 1) * v) / (2.0f * N));
        }
    }
...
void computeDCT2D_CUDA(const vector<float>& input, vector<float>& output) {
    const int N = RESIZE_W;
    ...
    cudaError_t err = cudaMemcpy(d_input, input.data(), size, cudaMemcpyHostToDevice);
    if (err != cudaSuccess) {
    ...
    dct2d_kernel<<<gridDim, blockDim>>>(d_input, d_output, N);
...
uint64_t computePerceptualHash(unsigned char* imgData, int width, int height, int channels) {
    ...
    computeDCT2D_CUDA(resizedGray, dctOutput);
